classdef UWDataProc < handle & FolderProc.GenericProc
    ...
    methods
        function obj = UWDataProc(opts, varargin)
    ...
    methods(Access=private)
        ...
        function folder_proc(obj, seq)
            ...
            bmask_image = image_list(~cellfun(@isempty, regexp(image_list, [model_name, '_\d+_\d+', obj.names.bmask, '\.png'], 'match')));
            nums = numel(depth_image);
            if nums ~= numel(color_image) || nums ~= numel(bmask_image)
                fprintf(2, 'depth/color image count unmatched!\n');
            end
            loc_info = image_list(~cellfun(@isempty, regexp(image_list, [model_name, '_\d+_\d+', obj.names.locxy, '\.txt'], 'match')));
            if nums ~= numel(loc_info)
                fprintf(2, 'depth/color image count unmatched!\n');
            end
            image_list = cat(2, depth_image, color_image, bmask_image, loc_info);
    ...
    methods(Static)
        function [im_d, im_c, im_a] = LoadImage(dir_pref, image_pair)
            ...
            im_c = imread(sprintf('%s/%s', dir_pref, image_pair{2}), 'png');
            im_d = imread(sprintf('%s/%s', dir_pref, image_pair{1}), 'png');
            if 2 == size(image_pair, 2)
                im_a = true(size(im_d));
            else
                im_a = imread(sprintf('%s/%s', dir_pref, image_pair{3}), 'png');
            end
            if 2 < ndims(im_d)
                im_d = rgb2gray(im_d);
            end
