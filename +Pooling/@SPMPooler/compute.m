...
if isempty(obj.encoder_)
    error('setup encoder first!');
end
...
pcode = zeros(obj.encoder_.get_output_dim(),bin_count);
...
for sx_bin = 1:obj.quad_divs
    for sy_bin = 1:obj.quad_divs
        ...
        if ~isempty(feats_sel)
            ...
            pcode(:,(sx_bin-1)*obj.quad_divs+sy_bin) = sum(code, 2);
        else
...
if obj.horiz_divs > 0
    ...
    for sy_bin = 1:obj.horiz_divs
        ...
        if ~isempty(feats_sel)
            ...
            if nnz(isnan(code)), error('Code contains NaNs'); end
            % accumulate histogram
            pcode(:,bin_quads_count+sy_bin) = sum(code, 2);
        else
            warning('SPMPool:EmptyBin','empty bin!');
        end
    end
